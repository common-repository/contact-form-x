	...
	if ($screen_id === 'settings_page_contactformx') {
		...
		if (!contactformx_check_date_expired() && !contactformx_dismiss_notice_check()) {
			...
			<div class="notice notice-success">
				<p>
					<strong><?php esc_html_e('Fall Sale!', 'contact-form-x'); ?></strong> 
					<?php esc_html_e('Take 25% OFF any of our', 'contact-form-x'); ?> 
					...
					<?php esc_html_e('Apply code', 'contact-form-x'); ?> <code>FALL2024</code> <?php esc_html_e('at checkout. Sale ends 12/21/24.', 'contact-form-x'); ?> 
...
function contactformx_dismiss_notice_link($tab) {
	...
	return '<a class="cfx-dismiss-notice" href="'. esc_url($href) .'">'. esc_html($label) .'</a>';
	
}

function contactformx_check_date_expired() {
	
	$expires = apply_filters('contactformx_check_date_expired', '2024-12-21');
